...
Ερώτηση:Να βρεθεί ο host_name με τα περισσότερα listings (σε απόλυτο αριθμό)(Μονάδες 10)
```{r}
...
Ερώτηση:Τι ποσοστό των host είναι superhost;(Μονάδες 10)
```{r}
#ok
...
Ερώτηση:Nα βρείτε τα ονόματα των καταλυμάτων τα οποία επισκέφτηκε ο χρήστης με τα περισσότερα reviews(Μονάδες 10)
```{r}
...
Ερώτηση:Με βάση τα στοιχεία του πίνακα reviews_detailed να βρείτε το πιο δημοφιλές κατάλυμα(συνολικός αριθμός reviews) και το όνομα του ατόμου που το οποίο πραγματοποίησε τις περισσότερες κριτικές για αυτό. (Μονάδες 10)
```{r}
...
Ερώτηση:Ποιος μήνας από τους καταγραφόμενους(βλέπετε πίνακας calendar) είχε τα περισσότερα διαθέσιμα δωμάτια; (Μονάδες 10)
```{r}
#ok
...
Ερώτηση:Βάση του πίνακα calendar και του reviews_detailed να βρείτε τα μέσα κέρδη(συνολικά έσοδα/πλήθος κατηγορίας) των καταλυμάτων ως προς τις κατηγορίες της στήλης host_response_time (Μονάδες 10)
```{r}
...
Ερώτηση:Να βρείτε τις μέσες τιμές(κόστος καταλύματος) για καταλύματα τύπου Entire home/apt τα οποία είχαν διαθεσιμότητα λιγότερο από 20 μέρες τον χρόνο (Μονάδες 5)
```{r}
#ok
...
Ερώτηση:Να βρείτε την μέση τιμή καταλυμάτων τύπου Entire home/apt στην περιοχή (neighbourhood_group) Arganzuela (Μονάδες 5)
```{r}
...
Ερώτηση:Να βρείτε τον συνολικό αριθμό reviews ανά έτος.Υπάρχει κάποιου είδους συσχέτιση του αριθμού των reviews με τον χρόνο;Να το δείξτε μέσω κατάλληλου διαγράμματος (Μονάδες 15)
```{r}
#ok
